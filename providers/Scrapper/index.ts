...
import DateUtils from "../DateUtils/index";
import ScrappingException from "App/Exceptions/ScrappingException";

const dateUtils = new DateUtils()
export default class Scrapper {
  private CALENDAR_URL_TO_SCRAP: string;

  constructor() {
    this.CALENDAR_URL_TO_SCRAP = Env.get('CALENDAR_URL_TO_SCRAP')
  }
  ...
  public async fetchWeek (firstname: string, lastname: string, queriedDate: string) {
    let result = null
    let success = false
  ...
  private async scrapWeek (firstname: string, lastname: string, queriedDate: object) {
    return new Promise((resolve, reject) => {

      request(`${this.CALENDAR_URL_TO_SCRAP}&Tel=${firstname}.${lastname}&date=${queriedDate}`, (err, resp, html) => {
        /* istanbul ignore if */
        if (err || !html || resp.statusCode !== 200) return reject(
          new ScrappingException('An error has occurred whilst trying to scrape the agenda', 500, 'E_SCRAPPING_AGENDA')
        )
        /* istanbul ignore if */
        if (html.includes('Erreur de parametres')) return reject(new ScrappingException('Parameter error', 500, 'E_SCRAPPING_PARAMETERS'))
        ...
        result = this.regroupCourses(result)
        resolve(result)
      })
    })
  }
  ...
  public async fetchDay (firstname, lastname, queriedDate) {
    let result = null
    let success = false
  ...
  private async scrapDay (firstname, lastname, date) {
    return new Promise((resolve, reject) => {
      request(`${this.CALENDAR_URL_TO_SCRAP}&tel=${firstname}.${lastname}&date=${date}%208:00`, (err, resp, html) => {
        /* istanbul ignore if */
  ...
  public async fetchTeamsLinks (firstname: string, lastname: string) {
    // get current date and format with moment
    const date = moment(new Date()).format('MM/DD/YYYY')

    return new Promise((resolve, reject) => {
      request(`${this.CALENDAR_URL_TO_SCRAP}&Tel=${firstname}.${lastname}&date=${date}`, (err, resp, html) => {
        /* istanbul ignore if */
        if (err || !html || resp.statusCode !== 200) return reject(new ScrappingException('An error has occurred whilst trying to scrape the agenda', 500, 'E_SCRAPPING_AGENDA'))
        /* istanbul ignore if */
        if (html.includes('Erreur de parametres')) return reject(new ScrappingException('Parameter error', 500, 'E_SCRAPPING_PARAMETERS'))
  ...
  private regroupCourses (result: object) {
    let response = {}
    const key = 'week'
    response[key] = {}
      ...
      for (let i = 0; i <= courses.length - 1; i++) {
        if (courses[i + 1] !== undefined) {
          ...
          if (courses[i].date === courses[i + 1].date && courses[i].subject === courses[i + 1].subject && courses[i].end === courses[i + 1].start && courses[i].professor === courses[i + 1].professor && courses[i].room === courses[i + 1].room && courses[i].weekday === courses[i + 1].weekday && courses[i].bts === courses[i + 1].bts) {
            ...
            response = this.pushCoursesUtil(response, key, course)
            i++
          } else {
            response = this.pushCoursesUtil(response, key, courses[i])
          }
        } else {
          response = this.pushCoursesUtil(response, key, courses[i])
        }
  ...
  private pushCoursesUtil (response: Array, key: string, course: object) {
